...
public class ItemRecommender {
    ...
    private List<Item> recommendByTopGames(ItemType type, List<Game> topGames) throws RecommendationException {
         ...
         * items are related to the top games provided in the argument
        ...
        for(Game game : topGames) {
            ...
            try {
                items = client.searchByType(
                        game.getId(), type, DEFAULT_PER_GAME_RECOMMENDATION_LIMIT
                );
    ...
    private List<Item> recommendByFavoriteHistory(
        ...
        Map<String, Long> favoriteGamIdByCount = favoriteGameIds
                .parallelStream()
        ...
        List<Map.Entry<String,Long>> sortedFavoriteGameIdListByCount = new ArrayList<>(favoriteGamIdByCount.entrySet());
        ...
        for (Map.Entry<String, Long> favoriteGame : sortedFavoriteGameIdListByCount) {
            ...
            try {
                items = client.searchByType(
                        favoriteGame.getKey(), type, DEFAULT_PER_GAME_RECOMMENDATION_LIMIT
                );
            } catch (TwitchException e) {
    ...
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommendationException {
        ...
        for ( Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet() ) {
            if (entry.getValue().isEmpty()) { // 说明没 like 过任何内容，所以从 top games 里拿一些
                TwitchClient client = new TwitchClient();
                List<Game> topGames;
                ...
                try {
                    ...
                    e.printStackTrace();
                    throw new RecommendationException("Failed to get top games for recommendation");
                }

               recommendedItemMap.put(
                       entry.getKey(),
                       recommendByTopGames(
                               ItemType.valueOf(entry.getKey()), topGames)
               );
            } else {
                ...
                recommendedItemMap.put(
                        ...
                        recommendByFavoriteHistory(
                                favoriteItemIds, entry.getValue(), ItemType.valueOf(entry.getKey())
                        )
                );
            }
        }
    ...
    public Map<String, List<Item>> recommendItemsByDefault() throws RecommendationException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new RecommendationException("Failed to get top games for recommendataions");
        }
